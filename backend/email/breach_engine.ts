...
export const BREACH_POOL: BreachRecord[] = [
...
const PATTERN_RULES = [
  { match: (email: string) => email.endsWith('@gmail.com'), breachNames: ['LinkedIn', 'Adobe'] },
  { match: (email: string) => email.endsWith('@yahoo.com'), breachNames: ['Yahoo', 'Zing ID'] },
  { match: (email: string) => email.startsWith('test@'),    breachNames: ['Canva', 'MyFitnessPal'] },
  { match: (email: string) => email.startsWith('admin@'),   breachNames: ['RockYou', 'Collection #1'] },
];

export interface LookupBreachOptions {
 ...
 * @param email
 * @param options
 * @returns BreachQueryResult
 */
export async function lookupBreaches(email: string, options: LookupBreachOptions = {}): Promise<BreachQueryResult> {
 ...
 */
async function queryHIBP(email: string, apiKey: string, session?: ScanSession): Promise<BreachQueryResult> {
  ...
  try {
    const response = await HttpFactory.fetchWithSession(
 ...
 */
function queryMockDatabase(email: string): Promise<BreachQueryResult> {
