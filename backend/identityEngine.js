...
const { getAllPlatforms } = require('./registry');
 ...
 * @param {string} realName - Already normalized to base ASCII by analyzer
 * @param {boolean} [deepScan] - Whether to generate secondary variants
 * @returns {string[]}
 */
 ...
 * @param {object[]} results - Platforms where a profile was FOUND
 * @param {string} realName - Target real name
 ...
 * @param {string} realName - Normalised target real name
 * @param {object} [options]
 * @param {boolean} [options.deepScan] - Enable deep variants
 * @param {object} [options.cookies] - Optional platform session cookies
 * @param {function} [options.onResult] - Callback on profile found
 * @param {function} [options.onProgress] - Callback on scan progress
 * @param {AbortSignal} [signal]
 * @returns {Promise<{found: object[], confidence: string}>}
 */
async function scanIdentity(realName, options = {}, signal = null) {
  ...
  const platformPromises = platformsToScan.map(async (platform) => {
    for (const variant of variants) {
      ...
      try {
        const result = await scanPlatform(variant, platform, options.cookies, signal);
