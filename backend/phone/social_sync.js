...
const { getDeterministicProfile } = require('./caller_id');

/**
 * Normalizes strings to create clean alphanumeric usernames by stripping accents and special characters.
 *
 * @param {string} str - Raw input name string
 * @returns {string} Clean alphanumeric string
 */
 ...
 * @returns {Promise<object>} SocialSyncResult
 */
async function discoverSocialProfiles(phone) {
  ...
  const ottProfiles = appList.map((app) => {
    return {
      app,
