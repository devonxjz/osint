...
const { analyzeInput } = require('../src/analyzer');
const { getPlatforms } = require('../src/registry');
...
describe('MVC Pipeline Integration', () => {
  it('should sanitize input and successfully resolve matching platform configurations', () => {
    ...
    expect(platformsToScan.length).toBe(19); // 10 Tech + 9 Gaming
  ...
  it('should resolve all platforms if categories array is empty or undefined', () => {
    ...
    expect(allPlatforms.length).toBe(45);
  });
  ...
  it('should gracefully stop scanning if input analyzer rejects the input format', () => {
    ...
    expect(platformsToScan.length).toBe(10); // Standard retrieval should still function normally without crashing
  });
});
