...
async function detectWildcardDns(domain) {
    ...
    try {
        const ips = await dns_1.promises.resolve4(randomSub);
        return new Set(ips);
    }
    catch (err) {
        return new Set();
    ...
    function parseEntities(entities) {
        for (const entity of entities) {
            ...
            if (entity.vcardArray && Array.isArray(entity.vcardArray[1])) {
                ...
                if (isRegistrant) {
                    if (nameVal && !registrantName)
                        registrantName = nameVal;
                    if (emailVal && !registrantEmail)
                        registrantEmail = emailVal;
                }
                else {
                    if (nameVal && !registrantName)
                        registrantName = nameVal;
        ...
        try {
            ...
            if (fileRes.data) {
                ...
                if (url.endsWith('.pdf')) {
                    ...
                    if (authorMatch)
                        docNode.author = authorMatch[1].trim();
                    if (emailMatch)
                        docNode.email = emailMatch[0].trim();
...
async function resolveDomainIntel(domain, options = {}, signal = null) {
    ...
    for (let i = 0; i < total; i++) {
        ...
        try {
            const ips = await dns_1.promises.resolve4(sub);
            const ip = ips[0];
