...
function queryMockDatabase(email) {
    ...
    for (const rule of PATTERN_RULES) {
        if (rule.match(normalized)) {
            const matched = exports.BREACH_POOL.filter(b => rule.breachNames.includes(b.name));
    ...
    const startIndex = seed % exports.BREACH_POOL.length;
    const selected = [];
    for (let i = 0; i < count; i++) {
        selected.push(exports.BREACH_POOL[(startIndex + i) % exports.BREACH_POOL.length]);
    }
