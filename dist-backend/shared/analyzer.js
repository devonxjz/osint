...
exports.ERRORS = {
    ...
    TRAVERSAL_DETECTED: 'Security alert: Path traversal traversal detected',
    INPUT_TOO_LONG: 'Security alert: Input exceeds maximum length of 100 characters'
};
// Zod Input Boundary Schema for Security Shield (operates on strings)
const inputSecuritySchema = zod_1.z.string()
    .min(1, { message: exports.ERRORS.EMPTY })
    .max(100, { message: exports.ERRORS.INPUT_TOO_LONG }) // Protect against ReDoS & Buffer Bloat
    .refine(val => !val.includes('..'), {
    message: exports.ERRORS.TRAVERSAL_DETECTED // Path traversal protection (blocks directory traversal attempts)
});
/**
...
function analyzeInput(input) {
    // Step 1: Guard null/undefined/non-string check first
    if (input === null || input === undefined || typeof input !== 'string') {
        return {
            type: null,
            valid: false,
            sanitized: '',
            error: exports.ERRORS.NOT_A_STRING
        };
    }
    // Step 2: Zod security boundary parse
    const securityParse = inputSecuritySchema.safeParse(input);
    if (!securityParse.success) {
        const errorMsg = securityParse.error.issues[0].message;
        return {
            type: null,
            valid: false,
            sanitized: input,
    ...
    if (MALFORMED_EMAIL_REGEX.test(trimmed)) {
        return {
            ...
            valid: false,
            sanitized: trimmed,
            error: exports.ERRORS.INVALID_FORMAT
        };
    }
    // ─── 1. EMAIL DETECTION ───
    ...
    if (sanitized.length < USERNAME_MIN) {
        return {
            ...
            valid: false,
            sanitized: sanitized,
            error: exports.ERRORS.USERNAME_SHORT
        };
    }
    if (sanitized.length > USERNAME_MAX) {
        return {
            ...
            valid: false,
            sanitized: sanitized,
            error: exports.ERRORS.USERNAME_LONG
        };
    }
